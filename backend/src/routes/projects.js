/**
 * Projects (§15).
 */
import { z } from 'zod';
import { crudPlugin } from '../lib/crud.js';

const projectBase = {
  client_id: z.string().nullable().optional(),
  deal_id: z.string().nullable().optional(),
  title: z.string().min(1).max(300),
  description: z.string().max(8000).nullable().optional(),
  status: z.enum(['planning', 'active', 'review', 'done', 'cancelled']).optional(),
  budget: z.coerce.number().int().min(0).nullable().optional(),
  currency: z.string().length(3).default('RUB'),
  deadline: z.coerce.number().int().nullable().optional(),
};
export default async function projectRoutes(fastify) {
  // The crudPlugin registers its own preValidation hook *inside its scope*, so
  // sibling routes below (contacts, tags, subtasks, …) must require auth
  // explicitly — otherwise request.user is null and they 500 (and would be
  // effectively unauthenticated, an IDOR hole).
  fastify.addHook('preValidation', async (request, reply) => {
    if (!request.user) await fastify.requireAuth(request, reply);
  });
  await fastify.register(
    crudPlugin({
      table: 'projects',
      columns: ['client_id', 'deal_id', 'title', 'description', 'status', 'budget', 'currency', 'deadline'],
      createSchema: z.object(projectBase),
      updateSchema: z.object(Object.fromEntries(Object.entries(projectBase).map(([k, v]) => [k, v.optional()]))),
      search: ['title', 'description'],
    })
  );
}
